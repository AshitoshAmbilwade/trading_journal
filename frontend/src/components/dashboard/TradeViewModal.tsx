...
const inr = (v: number | string | undefined | null) => {
  const num = v === undefined || v === null ? NaN : Number(v);
...
const calculateTradeMetrics = (trade: Trade) => {
  const entryPrice = Number(trade.entryPrice ?? 0) || 0;
  const exitPrice = Number(trade.exitPrice ?? 0) || 0;
  const quantity = Number(trade.quantity ?? 0) || 0;
  const pnl = Number(trade.pnl ?? 0) || 0;
  const brokerage = Number(trade.brokerage ?? 0) || 0;
  ...
  if (trade.entryDate && trade.exitDate) {
    const entry = new Date(trade.entryDate as string);
    const exit = new Date(trade.exitDate as string);
    ...
    if ("image" in obj) {
      ...
      if (img && typeof img === "object") {
        ...
        for (const k of candidates) {
          const v = (img as Record<string, unknown>)[k];
          const s2 = toStr(v);
          if (s2) return s2;
        }
      }
...
const PerformanceIndicator = ({ value, type = "percentage" }: { value: number; type?: "percentage" | "amount" }) => {
  ...
  return (
    ...
    >
      ...
      <span className="truncate">{type === "percentage" ? `${absValue.toFixed(2)}%` : inr(absValue)}</span>
    </div>
  );
};

const CurrencyDisplay = ({ value, className = "" }: { value: number | string; className?: string }) => {
  const num = Number(value);
  const isPositive = !Number.isNaN(num) && num >= 0;
  return <span className={`font-bold truncate ${isPositive ? "text-green-400" : "text-red-400"} ${className}`}>{inr(num)}</span>;
};

const NumberDisplay = ({ value, className = "" }: { value: number | string; className?: string }) => {
  return <span className={`font-bold truncate ${className}`}>{formatNumber(value)}</span>;
};
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const t = extractTradeFromResponse(res);
        ...
        // keep original console logging for debugging
        // eslint-disable-next-line no-console
        console.error("[TradeViewPage] load error:", err);
        if (mounted) setError((err as Error)?.message || "Failed to load trade");
      } finally {
    ...
    <div className="min-h-screen bg-black/80 backdrop-blur-xl text-white">
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          ...
          {trade && (
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <div
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-semibold backdrop-blur-sm border ${
                  pnlNumber >= 0 ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"
                }`}
              >
                <div className="flex items-center gap-1 sm:gap-2">
                  {pnlNumber >= 0 ? <TrendingUp className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" /> : <TrendingDown className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />}
                  <span className="hidden xs:inline">{pnlNumber >= 0 ? "Profitable" : "Loss"}</span>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 mt-1.5 sm:mt-2">
                      ...
                      <span className="text-xs text-gray-400 truncate">{formatNumber(metrics.quantity)} units</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600 min-w-0">
                    ...
                    <div className="flex flex-col xs:flex-row xs:items-end gap-1 xs:gap-2 min-w-0">
                      <p className={`text-lg sm:text-xl lg:text-2xl font-bold truncate ${metrics.pnl >= 0 ? "text-green-500" : "text-red-500"}`} title={inr(metrics.pnl)}>
                        {inr(metrics.pnl)}
                      </p>
                    ...
                    <p className="text-xs text-gray-500 mt-1 truncate" title={inr(metrics.brokerage)}>
                      Includes brokerage: {inr(metrics.brokerage)}
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600 min-w-0">
                    ...
                    {metrics.durationDays > 0 && (
                      <p className="text-xs text-gray-500 mt-1 truncate">{metrics.dailyReturnRate > 0 ? "+" : ""}{metrics.dailyReturnRate.toFixed(2)}% daily</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="bg-black/40 backdrop-blur-sm border border-gray-700 hover:border-cyan-500/40 transition-all rounded-xl p-4 sm:p-6">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {[
                      ...
                      { icon: <DollarSign className="h-3 w-3 sm:h-4 sm:w-4" />, label: "Exit Price", value: trade.exitDate ? inr(metrics.exitPrice) : "-" },
                      ...
                      const displayValue = typeof rawValue === "string" ? (item.capitalize ? (rawValue.charAt(0).toUpperCase() + rawValue.slice(1)) : rawValue) : String(rawValue);
                      return (
                        <div key={item.label} className="flex items-center justify-between py-2 border-b border-gray-600 last:border-b-0 min-w-0">
                          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                            <div className="p-1 sm:p-1.5 bg-gray-700 rounded-md sm:rounded-lg flex-shrink-0">{item.icon}</div>
                            <span className="text-xs sm:text-sm font-medium text-gray-400 truncate">{item.label}</span>
                          </div>
                          <span className={`text-xs sm:text-sm font-semibold truncate ml-2 ${item.highlight ? "text-cyan-400" : "text-white"}`} title={String(rawValue)}>
                            {displayValue}
                          </span>
                        </div>
                      );
                    ...
                    <div key={item.label} className={item.span ? "md:col-span-2" : ""}>
                      ...
                      <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600 min-h-[80px] sm:min-h-[100px]">
                        <p className="text-gray-200 whitespace-pre-wrap leading-relaxed text-xs sm:text-sm break-words">
                          {item.value ? item.value : <span className="text-gray-500 italic">No {item.label.toLowerCase()} provided</span>}
                        </p>
                      </div>
                    </div>
                  ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          ...
                          <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600">
                            <ul className="space-y-1.5 sm:space-y-2">
                              {(trade.aiAnalysis?.plusPoints ?? []).map((p: unknown, i: number) => (
                                <li key={i} className="flex items-start gap-2 sm:gap-3 text-gray-200 text-xs sm:text-sm">
                                  ...
                                  <span className="leading-relaxed break-words flex-1">{String(p)}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      {(trade.aiAnalysis?.minusPoints ?? []).length > 0 ? (
                        <div className="space-y-2 sm:space-y-3">
                          ...
                          <div className="bg-gray-800/50 rounded-lg p-3 sm:p-4 border border-gray-600">
                            <ul className="space-y-1.5 sm:space-y-2">
                              {(trade.aiAnalysis?.minusPoints ?? []).map((p: unknown, i: number) => (
                                <li key={i} className="flex items-start gap-2 sm:gap-3 text-gray-200 text-xs sm:text-sm">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    >
                      <div className="relative overflow-hidden">
                        <img src={imageUrl} alt={`Trade ${trade.symbol ?? "chart"}`} className="w-full h-40 sm:h-48 object-cover group-hover:scale-105 transition-transform duration-500" />
              ...
              <div className="bg-black/40 backdrop-blur-sm border border-gray-700 hover:border-cyan-500/40 transition-all rounded-xl p-4 sm:p-6">
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-center py-2 sm:py-3 border-b border-gray-600 min-w-0">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold flex-shrink-0 ml-2 ${trade.exitDate ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-green-500/10 text-green-400 border border-green-500/20"}`}>
      ...
      {previewOpen && imageUrl &&
        ...
        ReactDOM.createPortal(
          <div role="dialog" aria-modal="true" onClick={() => setPreviewOpen(false)} className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-3 sm:p-4 backdrop-blur-sm transition-all duration-300">
            <div onClick={(e) => e.stopPropagation()} className="relative max-w-[95vw] max-h-[90vh] flex items-center justify-center">
              <button
              ...
              <div className="relative">
                <img src={imageUrl} alt={`Full-size ${trade?.symbol ?? "trade"} chart`} className="max-w-[95vw] max-h-[90vh] object-contain rounded-xl shadow-2xl bg-gray-800" />
