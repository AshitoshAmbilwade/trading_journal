...
// --- PLAN MAPPING: Razorpay plan_id -> our tier + label ---
interface PlanMeta {
  tier: Tier;
  planLabel: string;
}
...
router.post("/razorpay", express.raw({ type: "*/*" }), async (req, res) => {
  ...
  try {
    ...
    // map plan -> tier/label if possible
    const planMeta = mapPlanFromId(planId);

    // Handle subscription creation / activation explicitly
    if (event === "subscription.activated" || event === "subscription.created") {
      ...
      user.subscriptionStart =
        user.subscriptionStart || new Date(subscriptionEntity?.current_start * 1000 || Date.now());
      user.subscriptionEnd = null;

      await user.save();
      console.info("Webhook: subscription activated for user", String(user._id));
